...
import ModalDel from  "./CarTypeModalDel";
...
class CarType extends Component {
    ...
    refreshList = () => {
      axios
        ...
        .then((res) => {
          this.setState({carTypeList: res.data})
        })
    ...
    handleSubmit = (item) => {
      ...
      if (item.carId) {
        ...
        axios
          //get from the address provided       
          .put(`/api/cartype/${item.typeId}/`, item)
          //refresh the list
    ...
    handleDelete = (item) => {
      ...
      axios
        ...
        .delete(`/api/cartype/${item.typeId}/`)
        .then((res) => this.refreshList());
    };
    ...
    renderItems = () => {
      ...
      const newItems = this.state.carTypeList;
            ...
            {newItems.map((item) => {
              return(
                <tr>
                  ...
                  <td>{item.weeklyCost}</td>
                  <td>{item.monthlyCost}</td>
                  <td>{item.lateFee}</td>
                  <td>{item.changeBranchFee}</td>    
                  <td><button 
    ...
    */
    render() {
      ...
      return (
        ...
        <h1 className="text-black text-uppercase text-center my-4">Car Type Database</h1>
        <div className="row">
          ...
          <div className="col-md-12 col-sm-10 mx-auto p-0">
            <div className="card p-3">
              <div className="d-flex justify-content-between">
                ...
                <button onClick={this.createItem} className="btn btn-success mr-2">Add Type</button>
                <button onClick={this.deleteItem} className="btn btn-danger mr-2">Delete Type</button>
              </div>
              <div>
