...
import Modal from "./RentalModal";
import ModalDel from  "./RentalModalDel";
...
class Rental extends Component {
    ...
    handleSubmit = (item) => {
      ...
      if (item.rentalId) {
        ...
        axios
          //get from the address provided       
          .put(`/api/rental/${item.rentalId}/`, item)
          //refresh the list
    ...
    handleDelete = (item) => {
      ...
      axios
        ...
        .delete(`/api/rental/${item.rentalId}/`)
        .then((res) => this.refreshList());
    };
    ...
    renderItems = () => {
      ...
      const newItems = this.state.rentList;
            ...
            {newItems.map((item) => {
              return(
                <tr>
                  ...
                  <td>{item.customer}</td>
                  <td>{item.employee}</td>
                  <td>{item.branchDep}</td>
                  <td>{item.branchArr} </td>
                  <td>{item.goldMember}</td>
    ...
    */
    render() {
      ...
      return (
        ...
        <h1 className="text-black text-uppercase text-center my-4">Rental Database</h1>
        <div className="row">
          ...
          <div className="col-md-12 col-sm-10 mx-auto p-0">
            <div className="card p-3">
              <div className="d-flex justify-content-between">
                ...
                <button onClick={this.createItem} className="btn btn-success mr-2">Add Rental</button>
                <button onClick={this.deleteItem} className="btn btn-danger mr-2">Delete Rental</button>
              </div>
              <div>
